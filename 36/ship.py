...
class Ship():
    ...
    def apear_coordinates(self):
        ...
        for temp in self.a_map.aid_list:
            ...
            rightTopX = temp[0] + TILE
            lefttBottomY = temp[1] + TILE
            # left top
            if (rightTopX >= self.rect.x >= temp[0]) and (lefttBottomY >= self.rect.y >= temp[1]):
                return True
            # right top
            elif (rightTopX >= self.rightTopX >= temp[0]) and (lefttBottomY >= self.rect.y >= temp[1]):
                return True
            # left bottom
            elif (rightTopX >= self.rect.x >= temp[0]) and (lefttBottomY >= self.lefttBottomY >= temp[1]):
                return True
            # right bottom
            elif (rightTopX >= self.rightTopX >= temp[0]) and (lefttBottomY >= self.lefttBottomY >= temp[1]):
                return True

        
        # walls
        
        for row in self.a_map.world_map:
            for x,y in row:
                rightTopX = x + TILE
                lefttBottomY = y + TILE
                # left top
    ...
    def check_top_wall(self):
        ...
        #for row in self.a_map.world_map:
        #for row in self.a_map.world_map:
        for x,y in self.a_map.world_map[int(self.rect.y/TILE)-1]: # upgrade
            rightTopX = x + TILE
            lefttBottomY = y + TILE
    ...
    def check_left_wall(self):
        self.rightTopX = self.rect.x + self.SIZE
        self.lefttBottomY = self.rect.y + self.SIZE  
        for y,x in self.a_map.world_map_left_to_right[int(self.rect.x/TILE)-1]: # upgrade
        #for row in self.a_map.world_map:
            #for x,y in row:
            rightTopX = x + TILE
            lefttBottomY = y + TILE
    ...
    def check_bottom_wall(self):
        self.rightTopX = self.rect.x + self.SIZE
        self.lefttBottomY = self.rect.y + self.SIZE
        for x,y in self.a_map.world_map[int(self.rect.y/TILE)+1]:
            rightTopX = x + TILE
            lefttBottomY = y + TILE
